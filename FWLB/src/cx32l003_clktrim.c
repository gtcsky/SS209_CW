...
void ClkTrim_ClkConfig(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Refclk,uint32_t Calclk)
{
  uint32_t tmpreg = 0;
	...
	/*Set Reference clock*/
	tmpreg  &= ~REFCLK_MASK;
	tmpreg  |= Refclk;						//REFCLK_SEL[2:0] 		选择参考时钟
	
	/*Set Calibration clock*/
	tmpreg  &= ~CALCLK_MASK;
	tmpreg  |= Calclk;						//CALCLK_SEL[1:0]		选择待校准时钟
...
void ClkTrim_StartCmd(CLKTRIM_TypeDef*CLKTRIMx,FunctionalState NewState)
{
   ...
   if(NewState != DISABLE)
	 ...
	 else
	 {
		  ...
		  CLKTRIMx->CR &= ~CLK_TMRIM_START;					//TRIM_START   开始校准
	 }
}
   ...
   if(NewState != DISABLE)
	 {
		  ...
		  CLKTRIMx->CR |= CLK_MON_ENABLE;			//监视模式使能寄存器。
	 }
	 else
	 {
...
void ClkTrim_ClkCmd(CLKTRIM_TypeDef*CLKTRIMx,FunctionalState NewState)
{
   ...
   if(NewState != DISABLE)
	 ...
	 else
	 {
		  ...
		  CLKTRIMx->CR &= ~CLK_REFCALCLK_ENABLE;				//CLKTRIM_CR.CLKEN=1     enable参考时钟和校验时钟
	 }
}
...
void ClkTrim_SetRcntValue(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Value)
{
	...
	/*Set value to REFCON register*/
	 CLKTRIMx->REFCON = Value;							//CLKTRIM_REFCON  配置参考计数器初值
}
...
void ClkTrim_SetCalValue(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Value)
{
   ...
   CLKTRIMx->CALCON = Value;											//CLKTRIM_CALCON	配置校准计数器溢出值
}
