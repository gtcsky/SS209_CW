...
void Enter_DeepSleep(void)
{
	 ...
	 *配置AWK功能
	 */
//	switchRemoteCtrlIRQ(DISABLE);
//	AWK_Config();
//	NVIC_EnableIRQ(AWK_IRQn); 							/*打开ARM中断使能*/
//	return;
//	deinitRemoteCtrlDecoder();
...
{
	LPTIMER_ClearITFlag(LPTIMER,LPTIMER_IT_FLAG);					//设定为10ms中断一次
	sysTimes.vSystemLIRC5ms++;
	sysTimes.vSystem5ms++;
	sysTimes.vTimerCnt++;
	if(sysTimes.vTimerCnt%10==0){
		sysTimes.fTime50ms=1;
	}
	if(sysTimes.vTimerCnt%20==0){
		sysTimes.fTime100ms=1;
	}
	if(sysTimes.vTimerCnt%100==0){
		sysTimes.fTime500ms=1;
	}
	if(sysTimes.vTimerCnt%200==0){
		sysTimes.fTime1s=1;
		sysTimes.vTimerCnt=0;
	}
	if(sysTimes.vSystem5ms>=USER_DOG_CONST){
		NVIC_SystemReset();
	}
	lPTIMER_FLAG = 0x01;
}
