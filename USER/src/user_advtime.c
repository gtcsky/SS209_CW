...
//}
 ...
 void ADVTIM_Reset(ADVTIM_TypeDef* TIMx)
{
	RCC->REGLOCK = 0x55AA6699;				//reg write protect diable
	if(TIMx==ADVTIM1)
		RCC->RERIRST |= 1<<10;					//ADVTIM1 reset
	else if(TIMx==ADVTIM2)
		RCC->RERIRST |= 1<<11;					//ADVTIM1 reset
	RCC->REGLOCK = 0x55AA669A;				//reg write protect diable
}

 void ADVTIM_ReleaseReset(ADVTIM_TypeDef* TIMx)
{
	RCC->REGLOCK = 0x55AA6699;				//reg write protect diable
	if(TIMx==ADVTIM1)
		RCC->RERIRST &= ~(1<<10);				//ADVTIM1 reset
...
//  TIM_OCInitStructure.TIM_Pulse = ADVTIM_TIM_CHANNEL2PULSE;		//设置PWM channel2  占空比相关参数//CCR2[15:0] 包含了装入当前捕获/比较 2 寄存器的值(预装载值) 当前捕获/比较寄存器参与同计数器 TIM1_CNT 的比较，并在OC2 端口上产生输出信号   ADVTIM_TIM_CHANNEL4PULSE/(ADVICE_TIM_PERIOD+1)
  ...
  TIM_OCInitStructure.TIM_OCNPolarity = TIM_OCNPolarity_Low;       		//设置CC1NP 输入/捕获 1 互补输出极性  0/1:	OC1 高/低电平有效；
  TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCIdleState_Reset;		//设置OIS1N  输出空闲状态 1(OC1N 输出)    0/1:	OC1N=0/1;
  TIM_OCInitStructure.TIM_Pulse = ADVTIM_TIM_CHANNEL2PULSE;				//设置PWM channel2  占空比相关参数//CCR2[15:0] 包含了装入当前捕获/比较 2 寄存器的值(预装载值) 当前捕获/比较寄存器参与同计数器 TIM1_CNT 的比较，并在OC2 端口上产生输出信号   ADVTIM_TIM_CHANNEL4PULSE/(ADVICE_TIM_PERIOD+1)
...
//	RCC->REGLOCK = 0x55AA6698;
	GPIOA->AFR&=	0xFFFFF00F;											//GPIOA 复用配置 		PA1.2.3均为普通IO
	GPIOA->AFR|=	0x00000110;											//GPIOA 复用配置 		PA1.2.3均为普通IO
	GPIOD->AFR&=  	0xF000FFFF;											//PD4,PD5,PD6为PWM输出
	GPIOD->AFR|=  	0x01100000;											//PD4,PD5,PD6为PWM输出
}
/**********************************************************************
 * 开启或关闭 某个或几个PWM输出口
 ***********************************************************************/
...
//	switchPwmChannel(CW_LED_PORT,CW_LED_PIN,DISABLE);
//}
//void onMotorChl(void){
//	switchPwmChannel(CW_LED_PORT,CW_LED_PIN,ENABLE);
//}
...
//		userPwmParaStruct.dutyArray[MW_LED_DUTY_INDEX]=vtDuty;
		TIMx->CCR2 = (u32)(vtDuty*(userPwmParaStruct.period+1));
	}
	else if(Channel==PWM_CH3){
//		userPwmParaStruct.dutyArray[CW_LED_DUTY_INDEX]=vtDuty;
